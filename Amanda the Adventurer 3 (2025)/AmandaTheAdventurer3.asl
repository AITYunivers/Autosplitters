...
startup
{
	...
	dynamic[,] _settings =
	{
		{ "StartingSplits", true, "Starting Splits", null, "The timer will automatically start depending on these settings"},
			{ "PrequelStart", false, "Start in Prequel (IL only)", "StartingSplits", null},
			{ "Act01Start", true, "Start in ACT 01", "StartingSplits", null},
			{ "Act02Start", false, "Start in ACT 02 (IL only)", "StartingSplits", null},
		...
		{ "TapesSplits", false, "Tapes Splits", null, "The splits will occur once you've started the new tape" },
			...
			{ "M1", true, "Memory Tape 1 - Rollercoaster Ride", "TapesSplits", null },
			{ "3", true, "Tape 3 - Healthy Food Helps Us Grow", "TapesSplits", null },
			{ "4", true, "Tape 4 - A Day At The Beach", "TapesSplits", null },
			{ "M2", true, "Memory Tape 2 - Best Beach Friends", "TapesSplits", null },
			{ "5", true, "Tape 5 - Scavenger Hunt", "TapesSplits", null },
			{ "6", true, "Tape 6 - When I'm Grown Up", "TapesSplits", null },
			{ "7", true, "Tape 7 - Let's Have a Party!", "TapesSplits", null },
			{ "M3", true, "Memory Tape 3 - 7th Birthday", "TapesSplits", null },
		...
		{ "IgnoreFirstTape", false, "Don't split on first tape", null, "Ensures that the first tape is not split upon,\nthis is to prevent an ~8 second first split." }
	};
...
start
{
	if (!current.MovementEnabled || old.MovementEnabled)
		return false;

	if (settings["PrequelStart"] && vars.Helper.Scenes.Active.Name == "ATA3-PRELUDE")
		return true;
	
	if (settings["Act01Start"] && vars.Helper.Scenes.Active.Name == "ATA3-ACT01")
		return true;
	
	if (settings["Act02Start"] && vars.Helper.Scenes.Active.Name == "ATA3-ACT02")
		return true;
}
...
{
	// Tapes Splitting
	if (current.VideoCheck != old.VideoCheck && old.VideoCheck == 0 && settings[current.VideoID.ToString()] && !vars.PlayedTapes.Contains(current.VideoID.ToString()))
    {
        vars.PlayedTapes.Add(current.VideoID.ToString());
		if (!settings["IgnoreFirstTape"] || vars.PlayedTapes.Count > 1)
        	return true;
    } 
	else if (vars.PlayedTapes.Count == 0 && old.TVCanClick == true && current.TVCanClick == false && settings[current.VideoID.ToString()])
	{
        vars.PlayedTapes.Add(current.VideoID.ToString());
		if (!settings["IgnoreFirstTape"] || vars.PlayedTapes.Count > 1)
        	return true;
